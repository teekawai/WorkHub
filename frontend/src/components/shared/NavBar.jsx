...
// transparent=true → absolute, overlay lên hero, nền trong suốt tối nhẹ
// transparent=false (default) → sticky, nền xanh solid
// isEmployer=true → fixed glassmorphic navbar cho employer zone
function NavBar({ currentUser, transparent = false, isEmployer = false }) {
    ...
    // Khi transparent: theo dõi scroll để chuyển về solid khi user cuộn qua hero
    useEffect(() => {
    ...
    // ── Tính class động cho wrapper nav ──────────────────────────────────────
    let navClass
    
    if (isEmployer) {
        // Employer zone: fixed glassmorphic navbar từ đầu
        navClass = "fixed top-0 inset-x-0 z-50 bg-blue-950/40 backdrop-blur-md border-b border-white/10 shadow-sm"
    } else if (transparent) {
        // Finder hero: transition từ transparent sang solid khi scroll
        navClass = `fixed top-0 inset-x-0 z-50 transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${
              scrolled
                  ? "bg-blue-700/95 backdrop-blur-md shadow-md"
                  : "bg-black/30 backdrop-blur-[2px]"
          }`
    } else {
        // Default: sticky solid navbar
        navClass = "sticky top-0 z-50 bg-blue-600 shadow-md"
    }
                ...
                <div className="hidden md:flex items-center gap-1 flex-1 justify-center">
                    {navLinks.map((link) => (
                        <Link
                            key={link.label}
                            to={link.to}
                            className="text-white/85 hover:text-white text-sm font-medium px-3 py-1.5 rounded-md hover:bg-white/10 transition-colors duration-150 no-underline"
                        >
                            {link.label}
                        </Link>
                    ))}
